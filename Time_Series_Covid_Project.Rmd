...
FLORIDA CASE Univariate ANALYSIS
...
mlp.fit = mlp(ts(newcases_fl$positiveIncrease),hd=10)
mlp.fit
plot(mlp.fit)
...
for( i in 1:(135-(trainingSize + horizon) + 1))
{
  mlp.fit = mlp(ts(newcases_fl$positiveIncrease[1:trainingSize]), hd = 5, comb = "median")
...
mlp.fit = mlp(ts(newcases_fl$positiveIncrease[54:123]), hd = 5)
forecasts = forecast(mlp.fit,h = 12)
  
ASE = mean((newcases_fl$positiveIncrease[124:135] -forecasts$mean)^2)
print(ASE)
